...
def analyze_parsing_errors():
    ...
    with open(log_file, encoding="utf-8") as f:
        ...
        for line in lines:
            ...
            match = re.search(
                r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}) - WARNING - Podcast: (.+?) - (.+)",
                line,
            )
            if match:
    ...
    sorted_podcasts = sorted(
        errors_by_podcast.items(), key=lambda x: len(x[1]), reverse=True
    )
...
def analyze_extraction_stats():
    ...
    if len(extractions) > 1:
        ...
        if episodes_list:
            print(
                f"   Promedio de episodios por extracción: {sum(episodes_list) / len(episodes_list):.1f}"
            )
        ...
        if songs_list:
            print(
                f"   Promedio de canciones por extracción: {sum(songs_list) / len(songs_list):.1f}"
            )
...
def generate_summary_report():
    ...
    print(
        f"   Errores de parsing: {'✅' if os.path.exists(parsing_log) else '❌'} {parsing_log}"
    )
