...
def analyze_logs():
    ...
    error_counts = {
        'Entrada inválida descartada': len(re.findall(r'Entrada inválida descartada', content)),
        'Entrada sin separador descartada': len(re.findall(r'Entrada sin separador descartada', content)),
        'No se pudo parsear': len(re.findall(r'No se pudo parsear', content)),
        'Entrada inválida (post-limpieza)': len(re.findall(r'Entrada inválida \(post-limpieza\)', content))
    }
    ...
    with open('logs/extraction_stats.log', 'r', encoding='utf-8') as f:
        content = f.read()
    ...
    new_parser_errors = error_counts['Entrada inválida descartada'] + error_counts['Entrada sin separador descartada']
    old_parser_errors = error_counts['No se pudo parsear'] + error_counts['Entrada inválida (post-limpieza)']
    ...
    if old_parser_errors > 0:
        improvement = ((old_parser_errors - new_parser_errors) / old_parser_errors) * 100
