...
def handle_search_songs(query: str):
    ...
    for res in results:
        ...
        print(
            f"  📻 Podcast: Popcasting {res['program_number']} ({res['podcast_date']})"
        )
...
def handle_search_artist(query: str):
    ...
    for res in results:
        print(
            f"  🎵 ({res['position']}) {res['song_title']} (en Popcasting {res['program_number']} - {res['podcast_date']})"
        )
...
def main():
    ...
    subparsers = parser.add_subparsers(
        dest="command", help="Comandos disponibles", required=True
    )

    # --- Comando 'run' ---
    run_parser = subparsers.add_parser(
        "run", help="Ejecuta el extractor para actualizar la base de datos."
    )
    ...
    search_subparsers = search_parser.add_subparsers(
        dest="search_target", help="Qué buscar", required=True
    )

    # Sub-comando 'search songs'
    search_songs_parser = search_subparsers.add_parser(
        "songs", help="Busca canciones por título o artista."
    )
    search_songs_parser.add_argument(
        "query", type=str, help="Texto a buscar en títulos de canción o artistas."
    )
    search_songs_parser.set_defaults(func=lambda args: handle_search_songs(args.query))

    # Sub-comando 'search artists'
    search_artists_parser = search_subparsers.add_parser(
        "artists", help="Busca todas las canciones de un artista."
    )
    search_artists_parser.add_argument(
        "query", type=str, help="Nombre del artista a buscar."
    )
    search_artists_parser.set_defaults(
        func=lambda args: handle_search_artist(args.query)
    )
