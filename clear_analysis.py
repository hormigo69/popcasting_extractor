...
def analyze_errors_by_date():
    ...
    for line in lines:
        ...
        if match:
            ...
            if (
                "Entrada inválida descartada" in line
                or "Entrada sin separador descartada" in line
            ):
...
def show_latest_errors():
    ...
    new_errors = re.findall(
        r"2025-07-18.*?Entrada (?:inválida|sin separador) descartada.*?(?=\n\d{4}-\d{2}-\d{2}|\Z)",
        content,
        re.DOTALL,
    )
