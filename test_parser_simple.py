                    ...
                    if not any(
                        ...
                        if not any(
                            ...
                            if validate_song_entry(cleaned_artist, cleaned_song):
                                ...
                                print(
                                    f"AVISO {program_info}: Entrada inválida descartada: '{artist} · {song}'"
                                )
                ...
                if len(part) > 3 and len(part) < 200:  # Longitud razonable
                    ...
                    if len(words) >= 2:
                        ...
                        if validate_song_entry(cleaned_artist, cleaned_song):
                            ...
                            print(
                                f"AVISO {program_info}: Entrada sin separador descartada: '{part}'"
                            )
...
def test_parser():
    ...
    playlist_links = parse_playlist_simple(
        test_description_with_links, "Episodio con enlaces"
    )
    ...
    playlist_special = parse_playlist_simple(
        test_description_special_chars, "Episodio con caracteres especiales"
    )
    ...
    playlist_only_links = parse_playlist_simple(
        "https://ko-fi.com/popcasting :: https://youtu.be/example", "Solo enlaces"
    )
