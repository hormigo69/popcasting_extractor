...
#!/usr/bin/env python3
"""
Script mejorado para extraer playlists desde WordPress que detecte el formato específico.
"""
...
def extract_playlist_from_html_v3(html_content):
    ...
    try:
        ...
        # Este patrón es el que usa Popcasting en sus páginas
        pattern = (
            r"([a-zA-Z0-9\s&\'\-\.]+)\s*·\s*([a-zA-Z0-9\s&\'\-\.\(\)]+)(?:\s*::\s*|$)"
        )
        ...
        # Si no encontramos nada con el patrón principal, buscar líneas que contengan múltiples canciones
        if not songs:
...
def main():
    ...
    Función principal.
    """
    print("🎵 Extractor de playlists desde WordPress (v3)")
    print("=" * 50)
