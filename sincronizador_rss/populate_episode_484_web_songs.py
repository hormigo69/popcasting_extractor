"""
Script para rellenar las canciones del episodio 484 desde los datos de la web.
"""
...
def populate_episode_484_web_songs():
    """
    Rellena las canciones del episodio 484 desde los datos de la web.
    """
    try:
        logger.info("=== RELLENANDO CANCIONES DEL EPISODIO 484 DESDE LA WEB ===")
        ...
        # 2. Obtener el episodio 484 de la BD
        logger.info("2. Obteniendo episodio 484 de la BD...")
        episode_484 = db_manager.get_podcast_by_program_number(484)
        
        if not episode_484:
            logger.error("❌ No se encontró el episodio 484 en la BD")
            return False
        
        logger.info(f"   ✅ Episodio encontrado: {episode_484.get('title', 'Sin título')}")
        logger.info(f"   ID del episodio: {episode_484.get('id')}")
        
        # 3. Verificar si ya tiene canciones
        logger.info("3. Verificando canciones existentes...")
        existing_songs = db_manager.client.table('songs').select('id').eq('podcast_id', episode_484['id']).execute()
        ...
        # 4. Obtener la playlist de la web desde la BD
        logger.info("4. Obteniendo playlist de la web desde la BD...")
        web_playlist = episode_484.get('web_playlist')
        
        if not web_playlist:
            logger.error("❌ El episodio 484 no tiene playlist de la web en la BD")
            logger.info("   Campos disponibles en el episodio:")
            for key, value in episode_484.items():
                if key != 'id' and value:
                    logger.info(f"   - {key}: {str(value)[:100]}...")
            return False
        
        logger.info(f"   ✅ Playlist web encontrada: {len(web_playlist)} caracteres")
        logger.info(f"   Contenido: {web_playlist[:200]}...")
        ...
        final_songs = db_manager.client.table('songs').select('*').eq('podcast_id', episode_484['id']).execute()
...
if __name__ == "__main__":
    success = populate_episode_484_web_songs()
    if success:
        logger.info("🎉 Canciones del episodio 484 rellenadas correctamente desde la web")
    else:
