#!/usr/bin/env python3
"""
Script de prueba final para descargar y subir 3 MP3 al NAS Synology.
Usa la clase SynologyClient probada y documentada.
"""
...
def main():
    ...
    print("🎵 PRUEBA FINAL: Descarga y subida de 3 MP3 al NAS")
    print("=" * 60)
    ...
    log_file = f"logs/mp3_test_3_final_{start_time.strftime('%Y%m%d_%H%M%S')}.json"
    ...
    try:
        with SynologyClient() as synology:
            ...
            # Verificar carpeta mp3
            print("📁 Verificando carpeta /mp3...")
            try:
                files = synology.list_files("/mp3")
                if files is not None:
                    print("✅ Carpeta /mp3 existe y es accesible")
                else:
                    print("⚠️  No se pudo listar /mp3, intentando crear...")
                    synology.create_folder("/mp3")
            except Exception as e:
                print(f"⚠️  Error verificando /mp3: {e}")
                print("📁 Intentando crear carpeta /mp3...")
                synology.create_folder("/mp3")
    ...
    with open(summary_file, 'w', encoding='utf-8') as f:
        f.write(f"PRUEBA FINAL - DESCARGA Y SUBIDA DE 3 MP3\n")
