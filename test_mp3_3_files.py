#!/usr/bin/env python3
"""
Script de prueba para descargar y subir 3 MP3 al NAS Synology.
"""
...
def main():
    """Función principal."""
    print("🎵 PRUEBA: Descarga y subida de 3 MP3 al NAS")
    print("=" * 50)
    ...
    # Log de resultados
    log_file = f"logs/mp3_test_3_files_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    with open(log_file, 'w', encoding='utf-8') as log:
        log.write(f"Log de prueba - 3 MP3 - {datetime.now()}\n")
        log.write("=" * 50 + "\n\n")
            ...
            if download_mp3(download_url, local_filename):
                ...
                try:
                    if synology.upload_file(str(local_filename), "/mp3"):
                        ...
                        log.write(f"✅ ÉXITO: Episodio #{program_number} - {remote_filename}\n")
                    else:
    ...
    if successful_uploads == len(df):
        ...
        print("⚠️  Algunos archivos fallaron. Revisa el log para más detalles.")
